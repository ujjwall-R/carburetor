...
**Storage**: N/A — stateless per invocation; config sourced from `megalodon.yml`  
...
class DeployCLI {
  ...
  private loadConfig(configPath: string): megalodonConfig
  private buildRequest(args: CLIArgs, config: megalodonConfig): DeploymentRequest
...
Set in `megalodon.yml` under `executor`:
