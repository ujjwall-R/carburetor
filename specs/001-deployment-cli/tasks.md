...
**Goal**: A developer can run `carborator deploy` to deploy a standard React or Node.js app from GitHub to AWS with a single command.

**Independent Test**: Clone a sample React repo, configure `carborator.yml` for AWS, run `carborator deploy`, verify the app is live at the returned URL.
...
**Independent Test**: Add a `Dockerfile` to a sample project, run `carborator deploy`, verify container is running in the cloud.
...
**Checkpoint**: `carborator deploy` detects Docker and runs a full container deploy independently of US1.

---

## Phase 5: User Story 3 — Custom Build/Deploy Script (Priority: P3)

**Goal**: User specifies a custom build script in `carborator.yml`; the tool runs it instead of the built-in default steps.

**Independent Test**: Set `project.build.script: "make build"` in config, run `carborator deploy`, verify the custom script is invoked and its output artifact is used.
...
**Checkpoint**: `carborator deploy` invokes custom scripts transparently. Standard React and Docker paths still work.
...
**Independent Test**: Change `target.platform` to `gcp` or `azure` in `carborator.yml` and run `carborator deploy` — app deploys to the correct cloud without any code change.
...
- [ ] T044 Implement `carborator version` subcommand in `src/client/DeployCLI.ts` — reads version from `package.json` and prints it
...
- [ ] T046 Build and smoke-test the compiled binary: `bun build --compile --outfile carborator src/index.ts`; run `./carborator --help` and `./carborator validate` against a test config
...
4. **STOP and VALIDATE**: run `carborator deploy` against a real React repo → AWS
5. Ship MVP
...
- Credentials are never read from `carborator.yml` — always from environment variables
