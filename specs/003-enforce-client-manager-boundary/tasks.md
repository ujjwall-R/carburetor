...
**Independent Test**: Run "`meg deploy --dry-run`" with valid and invalid credentials; verify exit code 0 / exit code 1 respectively. Confirm `grep -r "IShippingEngine" src/client/` returns no results.
...
**Independent Test**: Run "`meg validate`" with valid and invalid credentials; verify per-check output and exit codes are identical to pre-refactor behavior.
...
**Checkpoint**: Both `deploy --dry-run` and "`meg validate`" work correctly. `DeployCLI` has zero Engine references.
...
3. **STOP and VALIDATE**: Run "`meg deploy --dry-run`", confirm output, confirm no engine import
