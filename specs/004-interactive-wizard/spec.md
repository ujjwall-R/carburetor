...
**Independent Test**: A user can run `carburetor deploy --interactive`, answer every prompt, and observe a successful deployment (or validated dry-run) with zero `carburetor.yml` present.
...
**Independent Test**: Run `carburetor deploy --interactive --dry-run`, complete the wizard, and verify the output shows credential validation results with no deployment performed.
