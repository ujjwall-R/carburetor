...
**Independent Test**: A user can run "`meg deploy --interactive`", answer every prompt, and observe a successful deployment (or validated dry-run) with zero `megalodon.yml` present.

**Acceptance Scenarios**:

1. **Given** no `megalodon.yml` exists, **When** the user runs "`meg deploy --interactive`", **Then** the wizard starts and presents the first prompt (project type selection).
...
A user who wants to verify their interactive inputs without deploying runs "`meg deploy --interactive --dry-run`". The wizard collects all inputs and then runs credential validation only, reporting pass/fail for each credential.
...
**Independent Test**: Run "`meg deploy --interactive --dry-run`", complete the wizard, and verify the output shows credential validation results with no deployment performed.

**Acceptance Scenarios**:

1. **Given** the user runs "`meg deploy --interactive --dry-run`", **When** all prompts are answered, **Then** the system prints credential validation results and exits 0 on success — no deployment is performed.
...
- **FR-001**: "`meg deploy`" MUST accept a new `--interactive` flag that, when present, activates the wizard mode instead of requiring a config file.
...
- **SC-001**: A user with no `megalodon.yml` can complete an interactive deployment in under 3 minutes from running the flag to seeing the deployment outcome.
...
- **SC-005**: The existing file-based deploy path ("`meg deploy -c megalodon.yml`") produces identical results before and after this feature is added.
- **SC-006**: Running "`meg deploy --interactive --dry-run`" produces a credential-validation report without performing a deployment.
