...
This document defines the complete prompt sequence a user experiences when running "`meg deploy --interactive`". Each step shows the prompt type, label, available choices (for selects), validation rule, and which `DeploymentRequest` field it populates.
